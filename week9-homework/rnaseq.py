...
extra_filtered_fpkm = extra_filtered_fpkm[:,test5] #reorganizes columns to the list of sorted indexes
extra_filtered_fpkm = extra_filtered_fpkm[test2,:]
plt.imshow(extra_filtered_fpkm, cmap ="plasma", interpolation= "nearest", aspect= "auto")
# plt.setx genes
...
for i in range(extra_filtered_fpkm.shape[0]):
    ...
    for j in range(len(col_names)):
        list_of_tuples.append((rows[i],extra_filtered_fpkm[i,j], sexes[j], stages[j]))
...
mut_tests = multipletests(arr_p_values, method='fdr_bh') #need to filter the first index
